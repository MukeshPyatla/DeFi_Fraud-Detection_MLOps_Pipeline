"""
Main Streamlit app for DeFi Fraud Detection Pipeline - Streamlit Cloud Deployment
"""
...
def predict_fraud_demo(transaction_data: Dict[str, Any]) -> Dict[str, Any]:
    """Demo prediction function for when API is not available."""
    # Simulate prediction logic
    risk_score = 0.0
    
    # Calculate risk based on transaction features
    if transaction_data.get('value', 0) > 1.0:  # High value
        risk_score += 0.3
    if transaction_data.get('gas_price', 0) > 50:  # High gas price
        risk_score += 0.2
    if transaction_data.get('gas_used', 0) > 50000:  # High gas usage
        risk_score += 0.2
    ...
    return {
        ...
        'features_used': list(transaction_data.keys()),
...
def show_overview_page():
    ...
    with col1:
        st.metric(
            label="API Status",
            value=health_status.get('status', 'Demo Mode'),
            delta="✅" if health_status.get('status') == 'healthy' else "⚠️"
        )
    
    with col2:
        model_status = health_status.get('model_status', 'demo')
        st.metric(
            label="Model Status",
            value=model_status,
    ...
    if 'error' not in model_info:
        ...
        with col1:
            st.write(f"**Algorithm:** {model_info.get('algorithm', 'Demo Algorithm')}")
            st.write(f"**Version:** {model_info.get('version', 'Demo v1.0')}")
            st.write(f"**Feature Count:** {model_info.get('feature_count', '15')}")
...
def show_model_performance_page():
    ...
    if 'error' in model_info:
        st.warning("Using demo model data")
        model_info = {
    ...
    features = ['transaction_amount', 'gas_price', 'gas_used', 'block_time', 'sender_balance']
...
def show_prediction_page():
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            transaction_hash = st.text_input("Transaction Hash", 
            ...
            to_address = st.text_input("To Address", 
                                     value="0x742d35Cc6634C0532925a3b8D4C9db96C4b4d8b6")
            value = st.number_input("Value (ETH)", min_value=0.0, value=0.1, step=0.01)
        
        with col2:
            gas_price = st.number_input("Gas Price (Gwei)", min_value=0.0, value=20.5, step=0.1)
            gas_used = st.number_input("Gas Used", min_value=0, value=21000, step=1000)
            block_number = st.number_input("Block Number", min_value=0, value=15000000, step=1)
            timestamp = st.number_input("Timestamp", min_value=0, value=1640995200, step=1)
    ...
    if submitted:
        ...
        if risk_score > 0.7:
            st.error("⚠️ HIGH RISK - This transaction shows suspicious patterns")
        elif risk_score > 0.4:
            st.warning("⚠️ MEDIUM RISK - This transaction requires attention")
        else:
...
def show_settings_page():
    ...
    api_host = st.text_input("API Host", value=os.getenv('API_HOST', 'localhost'))
    api_port = st.number_input("API Port", value=int(os.getenv('API_PORT', 8000)), min_value=1, max_value=65535)
    ...
    with col1:
        health_status = get_api_health()
        st.write("**API Status:**", "🟢 Connected" if health_status.get('status') == 'healthy' else "🔴 Disconnected")
        st.write("**Model Status:**", "🟢 Loaded" if get_model_info().get('algorithm') else "🔴 Not Loaded")
    
    with col2:
        api_metrics = get_api_metrics()
        uptime = api_metrics.get('uptime_seconds', 0)
        st.write("**Uptime:**", f"{uptime / 3600:.1f} hours" if uptime > 0 else "N/A")
...
def main():
    ...
    if health_status.get('status') == 'healthy':
        ...
        st.sidebar.markdown('<div class="fraud-alert">⚠️ Demo Mode (API Disconnected)</div>', unsafe_allow_html=True)
        st.sidebar.info("This is a demo version. Connect to the API for full functionality.")
